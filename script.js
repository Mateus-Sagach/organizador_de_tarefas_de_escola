...
/*function gerarGrade() {
    ...
    const horarios = Array.from({ length: 12 }, (_, i) => `${i + 6}:00`);
    ...
    // Cabeçalhos com os dias da semana
    ...
    // Horários de 6:00 até 18:00 e as células correspondentes
    horarios.forEach(hora => {
        ...
        diasSemana.forEach(dia => {
            const div = document.createElement('div');
            div.id = `${hora}-${dia}`;
            div.innerHTML = `<strong>${hora} - ${dia}</strong>`;
...
// Função para gerar a grade de horários correta
/*function gerarGrade() {
    const horarios = Array.from({ length: 12 }, (_, i) => `${String(i + 6).padStart(2, '0')}:00`); // Formata os horários como 06:00, 07:00 etc.
    const gradeHorarios = document.getElementById('gradeHorarios');
    ...
    horarios.forEach(hora => {
        ...
        salas.forEach((_, index) => {
            const div = document.createElement('div');
            div.id = `${hora}-Sala ${index + 1}`;  // Gera o ID correto da célula
...
/*function cadastrarAtividade() {
    ...
    if (nomeAtividade && dia && hora && numFuncionarios > 0) {
        atividades.push({
            horario: hora,
            dia: dia,
...
// Atualizar a grade de horários com as atividades
/*function atualizarGradeComAtividades() {
    atividades.forEach(atividade => {
        const cell = document.getElementById(`${atividade.horario}-${atividade.dia}`);
        if (cell) {
            const funcionariosTexto = atividade.funcionariosAlocados.map((func, index) => {
                return `
                    <div>
                        ${func} <button class="remover" data-func-index="${index}" data-horario="${atividade.horario}" data-dia="${atividade.dia}">Remover</button>
                    </div>`;
            }).join('');
            ...
            let dropdown = `<select class="funcionario-disponivel" data-horario="${atividade.horario}" data-dia="${atividade.dia}">
            ...
            cell.innerHTML = `
                <strong>${atividade.horario} - ${atividade.atividade}</strong><br>
...
// Atualizar a grade para suportar múltiplas atividades por sala/horário
/*function atualizarGradeComAtividades() {
    atividades.forEach(atividade => {
        const cell = document.getElementById(`${atividade.horarioInicio}-${atividade.sala}`);
        if (cell) {
            const funcionariosTexto = atividade.funcionariosAlocados.map((func, index) => {
            ...
            cell.innerHTML += atividadeHtml; // Adiciona múltiplas atividades
        }
    });
}*/
